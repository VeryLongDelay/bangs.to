...
describe('parseCookie', () => {
  ...
  test('URL-encoded customUrl decoded correctly', () => {
    const s = parseCookie(
      req('suggest=custom,g,https%3A%2F%2Fapi.example.com%2Fsuggest%3Fq%3D%7B%7D')
    );
  ...
  test('cookie with frecency and custom sections — frecency ignored, custom works', () => {
    const s = parseCookie(req('suggest=google,g,|gh:50.yt:30|test8.mysite.proj'));
    expect(s.frecent).toEqual({});
    expect(s.custom).toEqual([]);
  });
  ...
  test('sf cookie with suggest= cookie together', () => {
    const s = parseCookie(req('sf=ddg:100.g:80; suggest=brave,b,|mdn:20|mysite'));
    expect(s.frecent).toEqual({});
  ...
  test('legacy suggest context is normalized on next response', () => {
    const { settings, rewrittenSuggestCookie } = parseSettingsFromRawUrlWithCleanup(
...
describe('readQueryParam', () => {
  ...
  test('finds query param in first, middle, and last position', () => {
    expect(readQueryParam('http://localhost/suggest?q=first&sp=ddg', 'q')).toBe('first');
    expect(readQueryParam('http://localhost/suggest?a=1&q=mid&b=2', 'q')).toBe('mid');
    expect(readQueryParam('http://localhost/suggest?sp=ddg&q=last', 'q')).toBe('last');
  });

  test('decodes + as space', () => {
    expect(readQueryParam('http://localhost/suggest?q=hello+world', 'q')).toBe('hello world');
  });

  test('decodes percent-encoded values', () => {
    expect(readQueryParam('http://localhost/suggest?q=%2Bcats%20dogs', 'q')).toBe('+cats dogs');
  });

  test('tolerates malformed percent-encoding like URLSearchParams', () => {
    expect(readQueryParam('http://localhost/suggest?q=%E0%A4%A', 'q')).toBe('�%A');
  ...
  test('returns first occurrence when repeated', () => {
    expect(readQueryParam('http://localhost/suggest?q=one&q=two', 'q')).toBe('one');
  });

  test('ignores fragment after query', () => {
    expect(readQueryParam('http://localhost/suggest?q=alpha#q=beta&sp=ddg', 'q')).toBe('alpha');
  });
});

describe('readTwoQueryParams', () => {
  test('returns both values from one query string', () => {
    expect(readTwoQueryParams('http://localhost/suggest?q=alpha&sp=ddg&x=1', 'q', 'sp')).toEqual([
      'alpha',
      'ddg'
    ]);
  });
  ...
  test('returns empty string for valueless keys', () => {
    expect(readTwoQueryParams('http://localhost/suggest?q&sp=', 'q', 'sp')).toEqual(['', '']);
  });

  test('returns null for missing keys', () => {
    expect(readTwoQueryParams('http://localhost/suggest?a=1', 'q', 'sp')).toEqual([null, null]);
  });

  test('uses first occurrence for repeated keys', () => {
    expect(
      readTwoQueryParams('http://localhost/suggest?q=first&q=second&sp=ddg&sp=bing', 'q', 'sp')
    ).toEqual(['first', 'ddg']);
  });
});
...
describe('parseSettingsFromRawUrl', () => {
  ...
  test('no sp query param falls back to cookie provider', () => {
    const s = parseSettingsFromRawUrl('http://localhost/suggest?q=cats', req('suggest=brave,b,'));
    expect(s.provider).toBe('brave');
    expect(s.trigger).toBe('b');
  });
  ...
  test('plain-mode parsing skips bang context while preserving core settings', () => {
    const s = parseSettingsFromRawUrl(
      'http://localhost/suggest?q=cats',
      req('suggest=custom,g,https%3A%2F%2Fexample.com%2Fsearch%3Fq%3D%7B%7D|meta|gh.mdn'),
      null,
      false
    );
    expect(s.provider).toBe('custom');
...
describe('bang suggestions — via suggest()', () => {
  ...
  test('"!gh" descriptions and suggestdetail stay aligned', async () => {
    ...
    const details = data[4]['google:suggestdetail'] as Array<Record<string, string>>;
...
describe('provider proxying — via suggest()', () => {
  ...
  test('provider=ddg → fetches duckduckgo suggest URL', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('https://duckduckgo.com/ac/?q=cats&type=list');
  });

  test('provider=bing → fetches bing suggest URL', async () => {
    fetchSpy.mockResolvedValueOnce(Response.json(['cats', []]));
    await suggest('cats', { ...defaultSettings, provider: 'bing' });
    expect(fetchSpy).toHaveBeenCalledWith('https://www.bing.com/osjson.aspx?query=cats');
  });

  test('provider=brave → fetches brave suggest URL', async () => {
    fetchSpy.mockResolvedValueOnce(Response.json(['cats', []]));
    await suggest('cats', { ...defaultSettings, provider: 'brave' });
    expect(fetchSpy).toHaveBeenCalledWith('https://search.brave.com/api/suggest?q=cats&rich=false');
  });
  ...
  test('provider=ecosia → fetches ecosia suggest URL', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('https://ac.ecosia.org/autocomplete?q=cats&type=list');
  });

  test('provider=kagi → fetches kagi suggest URL', async () => {
    fetchSpy.mockResolvedValueOnce(Response.json(['cats', []]));
    await suggest('cats', { ...defaultSettings, provider: 'kagi' });
    expect(fetchSpy).toHaveBeenCalledWith('https://kagi.com/api/autosuggest?q=cats');
  });

  test('provider=yandex → fetches yandex suggest URL', async () => {
    fetchSpy.mockResolvedValueOnce(Response.json(['cats', []]));
    await suggest('cats', { ...defaultSettings, provider: 'yandex' });
    expect(fetchSpy).toHaveBeenCalledWith('https://suggest.yandex.com/suggest-ff.cgi?part=cats');
  });
