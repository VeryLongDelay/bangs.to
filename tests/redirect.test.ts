...
vi.doMock('../src/generated/bangs-min.js', () => {
  ...
  BANGS.mdn = ['https://developer.mozilla.org/en-US/search?q=', '&topic=api&topic=js'];
  return {
...
describe('redirect — routing logic', () => {
  ...
  test('empty term with unparseable URL → fallback to replace', () => {
    const r = redirect('!g', settings({ custom: { g: splitUrl('not-a-url/{}') } }));
    expect(r.status).toBe(302);
...
describe('redirectRaw — routing logic', () => {
  ...
  test('empty term with unparseable URL → fallback to replace', () => {
    const r = redirectRaw('!g', settings({ custom: { g: splitUrl('not-a-url/{}') } }));
    expect(r.status).toBe(302);
    expect(loc(r)).toBe('not-a-url/');
  });
});
