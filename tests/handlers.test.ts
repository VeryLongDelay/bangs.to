...
describe('handleSuggestRequest', () => {
  test('returns 400 when q is missing', async () => {
    const response = await handleSuggestRequest(req('http://localhost/suggest'));
  ...
  test('uses sp query param as provider override', async () => {
    ...
    const response = await handleSuggestRequest(req('http://localhost/suggest?q=test&sp=ddg'));
