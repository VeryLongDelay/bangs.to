...
export async function initSettings(db: DB) {
  ...
  const [defaultBang, savedProvider, savedUrl, savedLucky, savedLuckyUrl, initialCustom] =
    await Promise.all([
  ...
  defaultInput.addEventListener('change', async () => {
    ...
    if (full[val]) {
      ...
      notifySW('invalidate');
      setSuggestCookie(suggestSelect.value, val, suggestUrlInput.value.trim(), custom);
  ...
  suggestSelect.addEventListener('change', async () => {
    ...
    notifySW('invalidate');
    setSuggestCookie(suggestSelect.value, defaultInput.value, suggestUrlInput.value.trim(), custom);
