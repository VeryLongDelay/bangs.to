...
function renderBangRow(trigger: string, bang: BangMeta) {
  ...
  row.append(
    ...
    el(
      "span",
      "text-xs text-text-secondary sm:text-right",
      bang.d || "Custom destination"
    )
  );
  return row;
}
  ...
  function render(query: string) {
    ...
    if (hits.length === 0) {
      results.replaceChildren(
        el("div", "py-4 text-center text-sm text-text-secondary", emptyText)
      );
      return;
    }

    results.replaceChildren(
      ...hits.map(([trigger, bang]) => renderBangRow(trigger, bang))
    );
  }
